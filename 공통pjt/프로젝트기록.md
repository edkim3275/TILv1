...
- searchresult페이지에서 home으로 안가짐 V

- mypage에서 다시 버튼클릭시 나는 오류수정 V
...
1. default사진을 보내는 기본 사진으로 설정. V
2. 지도테마변경 V
3. 첨부파일 모바일에서 뜨도록. V

## 210817

- 시연은 라이브로 ucc는 평가와 별개로 시상합니다.
...
- 서비스 설명 / 주요기능
  ...
  ```
  서비스 설명 : 당신의 발자취가 별이 되어 떠있는 곳: 루팅스타
  자신만의 알짜배기 루트를 작성하여 '특별한' 루트 이미지를 갖고, 
  다른 사람에게 공유하는 SNS 서비스.
  
  주요기능
  <정보를 얻을래!> : 메인페이지, 검색페이지, 댓글
   - 지도에 표시되는 경로를 기반하여 만들어진 게시글 조회
      - 작성자 정보표시, 경로기반 루트 이미지, 좋아요/저장
   - 태그를 이용한 검색
      - 무엇을, 누구와 2가지 태그를 통해 필터링 된 게시글 조회
      - 내가 작성한 게시글, 인기있는 게시글 조회
   - 댓글 기능
  
  <공유할래!> : 등록페이지
   - 구글 Map API기반으로 선택한 장소를 루트로 등록
   - 각 장소에 대한 세부정보 및 이미지 삽입 기능
   - 루트에 대한 태그(무엇을, 누구와) 기능
  
  <탐나는데!> : 저장, 수정페이지
   - 작성한 게시글 수정 기능
   - 다른 유저의 게시글 저장 기능
   - 저장한 게시글 내용을 수정하여 새로운 본인의 게시글로 등록
  ```

- 기술 특이점

  ```
   - User Guide페이지로 서비스 이해도 향상 도모 
   - 알람(FCM), 좋아요, 저장, 팔로우/팔로워
   - Canvas를 활용한 구글 Map 위 생성 된 경로를 루트 이미지로 변환기능
   - draggable기능을 활용한 리스트 위치 변경기능
   - amazon S3를 통해 이미지 업로드 및 활용
   - Spring Security를 이용한 소셜로그인(구글, 카카오)
   - JWT를 이용한 인증
   - HTTPS를 활용한 보안 강화
  ```
...
![image-20210817233337431](프로젝트기록.assets/image-20210817233337431.png)