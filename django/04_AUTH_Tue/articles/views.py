...
# not logged_in 일 경우에, 무조건 '/articles/login/'으로 redirect 한다.
@login_required # No login => DELETE /articles/new/요청에서 이게 위에있다면 unauthorized(401)
@require_http_methods(['GET', 'POST']) # 이게 위에있다면 no method(405)
def new(request):
    # 사용자 요청이 GET일 경우
    if request.method == 'GET':
        # 비어있는 새로운 form 생성
